...
def process_profile(profile_data):
    """Procesa un perfil: descarga imagen, calcula embedding con face-api, guarda ambos en DB"""
    username = profile_data.get("username", "unknown")
    name = profile_data.get("name", "Unknown")
    profile_image_url = profile_data.get("profile_image_url")
    face_embedding_from_json = profile_data.get("face_embedding")
    
    # Validaciones
    if not face_embedding_from_json:
        print(f"[SKIP] {name} ({username}): No tiene face_embedding en JSON")
        return False
    ...
    print(f"\n{'='*60}")
    print(f"Procesando (A/B Test): {name} ({username})")
    print(f"{'='*60}")
    
    # 1. Descargar imagen de perfil
    if not profile_image_url:
        print(f"[WARNING] {name}: No tiene URL de imagen, guardando solo embedding del JSON")
        face_encoding_faceapi = None
    else:
        temp_dir = tempfile.gettempdir()
        image_filename = f"{username}_profile.jpg"
        local_image_path = os.path.join(temp_dir, image_filename)
        
        print(f"[1/2] Descargando imagen desde {profile_image_url}...")
        downloaded_path = download_image(profile_image_url, local_image_path)
        
        if not downloaded_path or not os.path.exists(downloaded_path):
            print(f"[WARNING] {name}: No se pudo descargar la imagen")
            face_encoding_faceapi = None
        else:
            print(f"[OK] Imagen descargada: {downloaded_path}")
            
            # 2. Calcular embedding con face-api
            print(f"[2/2] Calculando embedding con face-api (128 dimensiones)...")
    ...
    try:
        ...
        face_encoding_json = face_embedding_from_json
        if not isinstance(face_encoding_json, list):
            face_encoding_json = list(face_encoding_json)
        ...
        data = {
            ...
            "face_encoding": face_encoding_json,  # Embedding del JSON (512 dimensiones)
        ...
        print(f"     - Embedding JSON: {len(face_encoding_json)} dimensiones")
        print(f"     - Embedding face-api: {len(face_encoding_faceapi) if face_encoding_faceapi else 'N/A'} dimensiones")
...
if __name__ == "__main__":
    ...
    if not os.path.exists(json_file):
        ...
        print(f"Uso: python upload_linkedin_data_ab_test.py [archivo.json]")
        exit(1)
    
    print("="*60)
    print("Iniciando carga A/B Test de perfiles de LinkedIn")
    print("Calculando embeddings con face-api además de usar el JSON")
    print("="*60)
    ...
    print("\nAhora ambos embeddings están guardados:")
    print("- face_encoding: embedding del JSON (512 dimensiones, método externo)")
    print("- face_encoding_faceapi: embedding calculado (128 dimensiones, face-api local)")
